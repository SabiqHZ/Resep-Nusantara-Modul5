            ...
            >
              ...
              <div className="relative bg-white/15 backdrop-blur-xl border border-white/25 rounded-2xl md:rounded-3xl overflow-hidden shadow-lg md:shadow-2xl shadow-green-500/5 hover:shadow-green-500/15 transition-all duration-500 cursor-pointer group-hover:scale-105 group-hover:bg-white/20">
                ...
                <button
                  ...
                  className="absolute top-3 right-3 z-30 bg-white/80 hover:bg-white rounded-full p-2 transition"
                >
                  <Heart
                    className={`w-5 h-5 transition-colors duration-200 ${
                      isFav ? "fill-red-500 text-red-500" : "text-slate-500"
                    }`}
                  />
                </button>

                {/* Gradient lembut hover */}
                <div className="absolute inset-0 bg-gradient-to-br from-green-100/10 via-green-200/10 to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-500 z-10" />

                {/* Gambar */}
                <div className="relative h-32 md:h-56 overflow-hidden z-0">
                  <img
                ...
                {/* 🔹 Overlay detail dengan fade in/out */}
                <div
                  className={`absolute inset-0 bg-black/80 text-white flex flex-col justify-center items-center p-4 md:p-6 text-sm md:text-base transition-opacity duration-500 ease-in-out overflow-y-auto z-20 ${
                    hoveredId === recipe.id
                ...
                >
                  ...
                  <p className="font-semibold text-green-300 mb-1">Bahan:</p>
                  <ul className="list-disc text-left text-gray-200 mb-3 ml-4">
                    {recipe.ingredients.slice(0, 4).map((bahan, i) => (
                      <li key={i}>{bahan}</li>
                    ))}
                  </ul>
                  <p className="font-semibold text-green-300 mb-1">Langkah:</p>
                  <ul className="list-decimal text-left text-gray-200 ml-4">
